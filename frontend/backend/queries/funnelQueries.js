...
function getVideoHeaderQuery(filter) {
  ...
  if (filter.predicates && filter.predicates.length) {
...
function getBreakdownQuery(filter, breakdownDimension) {
  return `
    WITH filtered_videos AS (
      SELECT DISTINCT rv."Video_ID"
      FROM raw_videos rv
    ...
    SELECT ${BREAKDOWN_EXPR_MAP[breakdownDimension]} AS label,
...
function getCompositionQuery(filter) {
  return `
    WITH filtered_videos AS (
      SELECT DISTINCT rv."Video_ID"
      FROM raw_videos rv
