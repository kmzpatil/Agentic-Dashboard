...
function createExplorerRouter(pool) {
  ...
  router.get('/multidim', async (req, res) => {
    ...
    try {
      ...
      return res.json({
        dim1, dim2, measure, timeGrain, dateField,
        dim1Value: dim1Value || null,
        matrixRows, dim1Values, dim2Values, timeSeriesRows,
      });
    } catch (error) {
  ...
  // Raw table browser — website_admin only
  ...
  router.get('/chart', async (req, res) => {
    ...
    try {
      ...
      if (aggregation === 'sum') {
        ...
        const result = await pool.query(getSumChartQuery(table, x, y));
        return res.json({ rows: result.rows });
      }

      const result = await pool.query(getCountChartQuery(table, x));
      return res.json({ rows: result.rows });
    } catch (error) {
      return res.status(500).json({ error: error.message });
    }
