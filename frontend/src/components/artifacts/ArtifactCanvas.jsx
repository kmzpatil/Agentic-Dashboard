...
// ── Report Tab ──────────────────────────────────────────────────────────────
function ReportTab({ xml }) {
  const parsed = useMemo(() => {
    if (!xml) return null;
    const parser = new DOMParser();
    const doc = parser.parseFromString(xml, 'text/xml');
    const title = doc.querySelector('title')?.textContent || 'Deep Research Report';
    const description = doc.querySelector('description')?.textContent || '';
    const sections = Array.from(doc.querySelectorAll('section')).map(s => ({
      title: s.getAttribute('title'),
      text: s.querySelector('text')?.textContent || '',
      chartRef: s.querySelector('chart_ref')?.getAttribute('id'),
    }));
  ...
  const downloadPDF = () => {
    ...
    const splitDesc = doc.splitTextToSize(parsed.description, 170);
    doc.text(splitDesc, 20, y);
    y += (splitDesc.length * 7) + 15;
    ...
    parsed.sections.forEach((s, i) => {
      ...
      const splitText = doc.splitTextToSize(s.text, 170);
    ...
    doc.text(`Generated by Frammer AI on ${new Date().toLocaleDateString()}`, 20, 285);

    doc.save(`${parsed.title.replace(/\s+/g, '_')}.pdf`);
  };

  if (!parsed) return (
    <div className="flex-1 flex items-center justify-center text-sm text-neutral-500 italic">
      Report data is still being processed or failed to generate.
    </div>
  );

  return (
    <div className="flex flex-col h-full bg-[#0D0D0D]">
      <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-800/60 bg-[#0F0F0F]">
        <h2 className="text-xl font-bold text-white tracking-tight">{parsed.title}</h2>
        <button
          onClick={downloadPDF}
          className="flex items-center gap-2 rounded-xl bg-red-600 px-4 py-2 text-sm font-bold text-white transition-all hover:bg-red-500 hover:scale-105 active:scale-95 shadow-lg shadow-red-900/20"
        >
          <Download size={16} /> Download PDF
        </button>
      </div>
      <div className="flex-1 overflow-y-auto p-8 max-w-4xl mx-auto w-full space-y-12">
        <div className="text-neutral-400 text-lg font-medium leading-relaxed border-l-2 border-red-500/30 pl-6 italic">
          {parsed.description}
        </div>
        {parsed.sections.map((s, i) => (
          <section key={i} className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700" style={{ animationDelay: `${i * 150}ms` }}>
            <h3 className="text-2xl font-bold text-neutral-100 flex items-center gap-3">
              <span className="h-8 w-1 bg-red-500 rounded-full" />
              {s.title}
            </h3>
            <div className="text-neutral-300 leading-8 text-[16px]">
              <ReactMarkdown>{s.text}</ReactMarkdown>
            </div>
          </section>
        ))}
        <div className="pt-12 border-t border-neutral-800 text-[11px] text-neutral-600 font-mono text-center">
          &copy; 2026 FRAMMER AI · ALL RIGHTS RESERVED · CONFIDENTIAL BUSINESS INTELLIGENCE
        </div>
      </div>
    </div>
  );
...
export default function ArtifactCanvas({ artifacts = [], datasets = [], sql = '', reportXml = '', onClose }) {
  ...
  const [activeTab, setActiveTab] = useState(reportXml ? 'report' : hasCharts ? 'visualize' : 'data');
  const [activeChartIndex, setActiveChartIndex] = useState(0);

  const activeChart = chartArtifacts[activeChartIndex] || chartArtifacts[0] || null;
  const panelTitle = activeChart?.title || tableArtifact?.title || (reportXml ? 'Research Report' : 'Analysis');

  useEffect(() => {
    if (reportXml) setActiveTab('report');
    else if (!hasCharts && activeTab === 'visualize') setActiveTab('data');
  }, [hasCharts, reportXml]);

  useEffect(() => { setActiveChartIndex(0); }, [chartArtifacts.length]);

  const tabs = [
    ...(reportXml ? [{ id: 'report', label: 'Report', icon: FileText }] : []),
  ...
  return (
    <div className="flex flex-col h-full bg-[#0B0B0B]">
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        {activeTab === 'report' && <ReportTab xml={reportXml} />}
