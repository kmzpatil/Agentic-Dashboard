      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {subTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-5 xl:grid-cols-[1.2fr_0.9fr_0.9fr]">
              {/* Check breakdown */}
              <Panel>
                      ...
                      <div key={key} className="group flex items-center gap-3 rounded-xl border border-neutral-800/60 bg-[#080808] p-3 transition-all duration-200 hover:border-neutral-700 hover:bg-[#0e0e0e]">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-between">
                            <span className="text-sm font-medium text-neutral-400 transition-colors group-hover:text-neutral-200">{conf.label}</span>
              ...
              {/* Dead Ends */}
              <Panel className="!border-amber-900/20 !bg-[#0d0b08]">
                <h3 className="mb-3 flex items-center gap-2 text-sm font-bold text-white">
                  <div className="rounded-lg bg-amber-500/10 p-1.5"><RouteOff className="h-3.5 w-3.5 text-amber-400" /></div>
                  Dead End Paths
                  <Badge className="ml-auto border border-amber-500/20 bg-amber-500/10 text-amber-400">{totalDeadEnds}</Badge>
                </h3>
                <div className="max-h-[300px] space-y-3 overflow-y-auto pr-1" style={{ scrollbarWidth: 'thin', scrollbarColor: '#262626 transparent' }}>
                  {Object.entries(deadEndGroups).map(([cat, items]) => (
                    <div key={cat}>
                      <div className="mb-1.5 text-[10px] font-bold uppercase tracking-[0.15em] text-neutral-600">{cat} ({items.length})</div>
                      <div className="space-y-1">
                        {items.map((item, idx) => (
                          <div key={idx} className="flex items-center justify-between rounded-lg border border-neutral-800/40 bg-[#0a0a0a] px-3 py-2 text-sm transition-all duration-150 hover:border-neutral-700/60 hover:bg-[#111111]">
                            <span className="truncate pr-3 font-medium text-neutral-300">{item.name}</span>
                            <div className="flex shrink-0 items-center gap-2 text-xs">
                              <span className="text-neutral-600">{item.uploads} uploads</span>
                              <span className="rounded-md bg-red-500/10 px-1.5 py-0.5 font-bold text-red-400">0 pub</span>
                            </div>
                          </div>
                        ))}
                      </div>
              ...
              {/* Suspicious Users */}
              <Panel className="!border-red-900/20 !bg-[#0d0808]">
                <h3 className="mb-3 flex items-center gap-2 text-sm font-bold text-white">
                ...
                <div className="max-h-[300px] space-y-2 overflow-y-auto pr-1" style={{ scrollbarWidth: 'thin', scrollbarColor: '#262626 transparent' }}>
                  {(data.suspicious_users || []).map((user, i) => {
                    ...
                    const cardBorder = user.severity === 'Critical' ? 'border-red-900/30 hover:border-red-800/40' : 'border-neutral-800/40 hover:border-neutral-700/60';
                    return (
                      <div key={i} className={`rounded-xl border ${cardBorder} bg-[#0a0a0a] p-3 transition-all duration-200 hover:bg-[#111111]`}>
                        <div className="mb-1.5 flex items-start justify-between gap-2">
                          <span className="truncate text-sm font-bold text-neutral-200">{user.name}</span>
                          <Badge className={`shrink-0 border ${sevClass}`}>{user.severity}</Badge>
                        </div>
                        <div className="mb-2.5 text-[11px] leading-relaxed text-neutral-500">{user.reason}</div>
                        <div className="flex gap-1.5 font-mono text-[10px]">
                          ...
                          <span className="rounded-md bg-neutral-800/60 px-2 py-0.5 text-neutral-500">P:{user.published}</span>
                        </div>
                      </div>
                    );
                  })}
