...
api_server2.py
──────────────
Temporary FastAPI server that exposes the Groq-powered Frammer analytics agent.
Cloned from api_server.py but targets agent_groq.py.
"""
...
app = FastAPI(title="Frammer Analytics API (Groq Version)", version="5.0.0-groq")
...
logger = logging.getLogger("frammer.api_server2")
...
async def chat(req: ChatRequest):
    logger.info("Chat request (Groq): %r (conv=%s)", req.message, req.conversation_id)
    ...
    if not conv:
        conv = create_conversation(title="New conversation (Groq)")
        conv_id = conv["id"]
