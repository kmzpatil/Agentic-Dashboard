...
def get_quality_overview(auth: AuthContext = Depends(require_auth)):
    try:
        ...
        issues.extend(_check_nulls())
        issues.extend(_check_duplicate_pks())
        issues.extend(_check_fk_violations())
        issues.extend(_check_negative_values())
        issues.extend(_check_invalid_dates())
        issues.extend(_check_unknown_values())
...
def _get_dead_ends() -> list[dict]:
    ...
    for row in result.rows:
        ...
        pub = query("""
            ...
            WHERE rv."Language" = $1
        """, [lang])
        if (int(pub.rows[0]["cnt"]) if pub.rows else 0) == 0:
            dead_ends.append({
                "category": "Languages",
                "name": lang,
    ...
    # By Platform (platforms with no valid published_posts backing)
    result = query("""
...
def _get_suspicious_users() -> list[dict]:
    ...
    # 2. Users where created > uploaded (anomaly)
    result = query("""
